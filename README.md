...
Este proyecto consiste en una **API REST robusta** diseñada para la gestión integral de libros electrónicos (E-books). El sistema permite administrar catálogos bibliográficos, gestionar autores y categorías, y simular el flujo de descargas de archivos digitales.

Desarrollado bajo un enfoque de **Ingeniería de Software Senior**, este sistema se presenta como un **Producto Mínimo Viable (MVP) centrado exclusivamente en el Backend**. Esta decisión estratégica permite que un único desarrollador asuma con éxito una carga de trabajo originalmente planificada para un equipo de cuatro integrantes, priorizando la lógica de negocio y la calidad del código sobre la interfaz de usuario.

## 🛠️ Stack Tecnológico
*   **Lenguaje:** [Go (Golang)](https://go.dev/) - Seleccionado por su eficiencia y soporte para concurrencia.
*   **BaaS (Backend as a Service):** [Supabase](https://supabase.com/) - Provee la base de datos PostgreSQL, autenticación JWT y almacenamiento de archivos (Storage).
*   **Librerías Core:** 
    *   `net/http` para el servidor web.
    *   `google/uuid` para identificadores inmutables.
    *   SDK oficial de Supabase para Go.

## 🏗️ Arquitectura del Sistema
El proyecto sigue una **Arquitectura Modular Limpia**, separando las responsabilidades para facilitar el mantenimiento y la escalabilidad:
